...
def get_user_choice():
    ...
    user_input = ''
    while (not (re.compile(r"^(?!.*(\d).*\1)[1-6](?: [1-6](?!.*\1)){0,5}$").match(str.strip(user_input))) and (user_input != 'a')): 
        user_input = input(f'Please select one or more defect types from ("a" for all):\n{defects}\n(separated by blank spaces, no duplicates):')
    if user_input == 'a':
        chosen_defects = list(map(lambda x: defects[x], range(1,7)))
    else:
...
image_pool_path = working_path + '../../../VOC_PCB/JPEGImages/'
image_dest_path = working_path + '../../data/Images/'
annot_pool_path = working_path + '../../../VOC_PCB/Annotations/'
