...
The size of our validation set is 20\% of the cropped, separated, balanced and unaugmented original training set. This means there are 413 samples. The evaluation metrics are based on these samples. The defect ratios can be seen in Fig.~\ref{fig:val_set}.
\begin{figure}[H]
    \begin{center}
        \begin{tabular}{|c|c|}
            \hline
...
The graphs in Fig.~\ref{fig:graphs} show the development of different metrics during the 26 training epochs. Validation accuracy for segmentation is above 93\% already after the first training epoch and reaching 97\% after the last one. It is not before the 10th epoch that the validation accuracy for classification is reaching 70\%. Same holds for classification precision and recall.
%\newgeometry{left=-3cm, right=-3cm, top=-3cm, bottom=-3cm}
\begin{figure}[H]
    \centering
    %\begin{turn}{270}
...
The confusion matrix (See Fig.~\ref{fig:confusion}) and the classification report (See Fig.~\ref{fig:classification_report}.) on the validation set show that precision and recall for each defect class vary greatly. 98\% of the shorts detections on the test set are correct, while 24\% of the detected spurious copper belong to a different class. Likewise, 98\% of the missing holes and shorts are correctly identified, while 20\% of the mouse bites are not or wrongly detected.

%\newgeometry{left=-3cm, right=-3cm, top=-3cm, bottom=-3cm}
\begin{figure}[H]
    \centering
    %\begin{turn}{270}
    \includegraphics[width=0.7\textwidth]{./graphics/confusionmatrix.png}
    %\end{turn}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{./graphics/classification_report.png}
...
Fig.~\ref{fig:results1} and Fig.~\ref{fig:results2} illustrate that the location of the defects or the pixel matrix is predicted quite precisely. For illustration purposes the real and predicted classes are given here, too.
...
The model works very well on our chosen dataset with a segmentation and classification accuracy of about xx\% and yy\%, respectively. While the classic U-NET architecture was sufficient to achieve
already high metric values in the segmentation, it was not before we combined it with residual blocks in each encoding and decoding step of the UNET (RES-UNET) that the accuracy of the
classification branch moved to acceptable levels. 
\clearpage
\newpage
